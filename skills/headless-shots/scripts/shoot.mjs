...
const chrome = spawn(process.env.CHROME || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", [
  ...
  `--remote-debugging-port=${port}`, `--user-data-dir=${tmpdir()}/headless-shots-prof`, "--window-size=1440,900", "about:blank",
], { stdio: "ignore" });
...
await call(ws, "Network.setCookie", { name: COOKIE, value: token, domain: new URL(BASE_URL).hostname, path: "/", httpOnly: true });
await call(ws, "Emulation.setDeviceMetricsOverride", { width: 1440, height: 900, deviceScaleFactor: 1, mobile: false });
for (const p of paths) {
  await call(ws, "Page.navigate", { url: `${BASE_URL}${p}` });
  await sleep(2500);
